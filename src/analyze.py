...
def individual(N_sn, N_s):
	...
	plt.plot(logposterior[0][200:])
	plt.plot(logposterior[1][200:])
	plt.plot(logposterior[2][200:])
	plt.plot(logposterior[3][200:])
	plt.show()
	# plt.plot(extract['w'])
	# plt.show()
	# plt.hist(extract['w'])
	...
	if missed_wrong.sum !=0:
		plt.plot(extract['zs_true_mis'][:,missed_wrong] - info['zs'][info['s_mis']][missed_wrong])
		plt.show()

...
transformed parameters{
  ...
  for (m in 1:N_s_obs){
    adu_obs[m] <- ((1+zs_obs[m])* splint(z_int_s, luminosity_distance_int_s, y2, n_int+1, zs_obs[m]))^(-2);
  }

  for (m in 1:N_s_mis){
    adu_mis[m] <- ((1+zs_mis[m])* splint(z_int_s, luminosity_distance_int_s, y2, n_int+1, zs_mis[m]))^(-2);
  }

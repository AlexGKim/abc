...
pgm = PGM([9.5, 6.5], origin=[0., 0.2], observed_style='inner')

pgm.add_node(Node('dispersion',r"\center{$\sigma_{Ia}$ \newline $\sigma_{non-Ia}$}", 1,6,scale=1.2,aspect=1.8))
pgm.add_node(Node('rate',r"$\theta_r$",2,6))
pgm.add_node(Node('theta_T',r"\center{$\alpha_{Ia}$, $\alpha_{non-Ia}$ \newline $\beta_{Ia}$, $\beta_{non-Ia}$}", 4,6,scale=1.4,aspect=1.8))
...
pgm.add_node(Node('theta_Ti',r"\center{$\theta_{Ia,i}$ \newline $\theta_{non-Ia,i}$}", 1,4,scale=1.5,aspect=1.2))
pgm.add_node(Node('Type',r"$T_i$", 2, 4))
pgm.add_node(Node('Gals',r"$\theta_G$", 9,4, observed=True))

pgm.add_node(Node('Luminosity',r"$L_i(t,\lambda)$", 4, 3, fixed=True, scale=1.2, offset=(-20,-25)))
...
pgm.add_node(Node('Flux',r"$f_i(t,\lambda)$", 6, 2, scale=1.2,fixed=True,offset=(20,-25)))
pgm.add_node(Node('Counts',r"$\overline{\mathit{ADU}}_i$", 7, 2,scale=1.2,fixed=True,offset=(-20,0)))
pgm.add_node(Node('Counts_g',r"$\overline{\mathit{ADU}}_{gi}$", 8, 2,scale=1.2,fixed=True,offset=(-25,0)))
...
pgm.add_node(Node('^Counts',r"${\mathit{ADU}_i}$", 8, 1, observed=True,scale=1.2,aspect=1.2))
...
# Big Plate: Galaxy
pgm.add_plate(Plate([0.5, 0.5, 8, 5.],

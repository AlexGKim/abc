...
mass, sSFR, etc.; and the sample-selection score Select.
...
\begin{itemize}
\item  $g_i$ labels the distinct galaxy that is the transient host.
\item $z_i$, $\theta_{gi}$ are the galaxy redshift and parameters (e.g.\ mass, sSFR, metallicity).
Given the galaxy $g_i$, these values are specified.
...
\begin{itemize}
\item ${\mathit{ADU}}$ is the realized flux. 
\item Select is the score used to decide if the transient is used in the analysis.
It and $\mathit{ADU}$ are not independent so what is relevant is their joint distribution
\begin{equation}
P({\mathit{ADU},\text{Select}} | \overline{\mathit{ADU}}, 

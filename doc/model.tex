...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=0.6]{/Users/akim/project/abc/results/{contour.100}.pdf} 
   \caption{Probability distribution function for $\Omega_M$--$w$  when 50\% of transients
   get spectra while active.
...
A transient
without a spectrum has uncertain type and uncertain redshift: the redshift
of the associated host galaxy, and redshifts associated with SNe~Ia and non-Ia's
at the observed magnitude, are viable but disjoint possibilities.  The initial condition
for redshifts of transients without an active spectrum is near the true redshift
for chains 0 and 2, and near the host redshift in chains 1 and 3.

Figure~\ref{missed_wrong.100:fig} shows draws
for the redshift parameter for two transients: one is Type~Ia and the other is
non-Ia, neither have a spectrum and both have an incorrect host galaxy association.
The SN~Ia is placed at $z=0.95$ but is misassociated with a galaxy at
$z=1.24$.  The posterior shows a preference for the input redshift, but as the analyzer
must entertain the possibility it is non-Ia the posterior sometimes
goes to lower redshifts. Although two of the chains start near the incorrect-host redshift,
there are no such redshifts drawn.

\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=0.6]{/Users/akim/project/abc/results/{missed_wrong.100}.pdf} 
   \caption{Redshift posterior for a Type~Ia and a non-Ia that were not observed
   spectroscopically and for whom the host-galaxy is misassociated.
   The results of the four chains are
   placed in sequence.   \label{missed_wrong.100:fig}}
\end{figure}
...
if were a Type~Ia.  Clearly this chain does not mix with the others.

Figure~\ref{posterior.100:fig} shows the log-posteriors and histograms of
the $w$-draws from Chain 2 compared to the others.
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=0.5]{/Users/akim/project/abc/results/{posterior.100}.pdf} 
   \includegraphics[scale=0.5]{/Users/akim/project/abc/results/{wsub.100}.pdf} 
   \caption{Log-posterior and $w$-draws from  Chain 2 compared to the others.  \label{posterior.100:fig}}
\end{figure}

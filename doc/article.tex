...
\begin{multline}
\sum_T \int d\theta_{Ia}\, d\theta_{!Ia}\, dL\, p(n, {{T}}_S,{{z}}_S, \theta_{S}, G | T, \theta_{Ia},
\theta_{!Ia}, L, \mathcal{S}, \Omega_M, w, r, \Theta_{Ia}, \Theta_{\mathit{!Ia}}, Z, g) \\
p(L|T, \theta_{Ia},
\theta_{!Ia},  \mathcal{S}, \Omega_M, w, r, \Theta_{Ia},\Theta_{\mathit{!Ia}}, Z, g)
p(T, \theta_{Ia},
\theta_{!Ia}|  \mathcal{S}, \Omega_M, w, r, \Theta_{Ia},\Theta_{\mathit{!Ia}}, Z, g),
\label{likelihood2:eqn}
\end{multline}
whose relations are sketched in the probabilistic graphical model in Figure~\ref{pgm:fig}.
$\mathcal{S}$ indicates that this likelihood corresponds to the analysis sample. 
For shorthand we asign $A=\{n, {{T}}_S,{{z}}_S, \theta_{S}, G \}$, $B=\{T, \theta_{Ia},
\theta_{!Ia}, L\}$, and $C=\{\Omega_M, w, r, \Theta_{Ia}, \Theta_{\mathit{!Ia}}, Z, \}$
so that the likelihood is more compactly expressed as $\int dB\, p(AB| \mathcal{S}, C)p(B| \mathcal{S}, C)$.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   ...
   ${X}_S$.  The properties of the galaxy identified as the host galaxy is $G$. 
   \label{pgm:fig}}
\end{figure}

The measurements are
\begin{itemize}
\item $n$: the measured counts for each photometric observation for all transients in the sample;
\item ${{T}}_S$, ${{z}}_S$, $\theta_S$: the spectroscopic type, redshift, and transient parameters available for a subset of transients;
\item $G$: measured properties of the galaxy identified
as the host.
\end{itemize}
The top-level model parameters are
\begin{itemize}
\item $\Omega_M$, $w$: the mass density and dark-energy equation of state parameters of a flat,
constant-$w$ dark-energy cosmology;
\item $r$:  the parameters
that describe the redshift-dependent relative fraction of SNe~Ia and non-Ia's;
\item $\Theta_{Ia}$, $\Theta_{\mathit{!Ia}}$:  All the fundamental parameters used to describe
the SN~Ia and non-Ia populations respectively, such as the mean
absolute magnitude, intrinsic magnitude dispersion, the distribution of internal parameters
(i,e,\ the distributions of SALT2's $x_1$ and $c$), the relationship between internal
parameters and magnitude (i.e.\ SALT2's $\alpha$, $\beta$);
...
including redshift $g_z$.
\end{itemize}
The second-level parameters are
\begin{itemize}
%\item $z$: the cosmological redshifts of the host and neighboring galaxies;
\item $T$: the class of the objects, SN~Ia or non-Ia;
\item $\theta_{Ia}$, $\theta_{!Ia}$: the transient parameters that describe individual
transients (i.e..\ SALT2's $x_1$, $c$);
\item $L(t,\lambda)$: the luminosity of the transient.
\end{itemize}
...
redshift.
%\item{$\overline{n}$}: the expected counts of the transient given
%the flux and transmission.
\end{itemize}

The relationships between the parameters are now described:
\begin{itemize}
...
\begin{equation}
\theta_{Ia} | \Theta_{Ia}, g \sim \mathcal{N}(0, \Theta_{ia, \theta}),
\end{equation}
where $\theta_{Ia} =\{x_1, c\}$ are the SALT2 light-curve shape and color parameters
and $\Theta_{ia, \theta}$ describes the covariance of their distributions.
...
%\begin{equation}
%\theta_{non-Ia} | \sigma_\beta, g  \sim \mathcal{N}(0, \sigma_{\beta, non-Ia}),
%\end{equation}
...
\begin{equation}
\theta_{S} (\theta_{Ia}, \theta_{!Ia}, T)=\theta_{T}
\end{equation}
...
\begin{equation}
T | r, g \sim \rm{Bernoulli}(\pi(r,g,T)),
\end{equation}
...
\begin{equation}
L(t,\lambda) |  \Theta_{Ia}, \theta_{Ia}, g \sim L_{SALT2}(t-t_0; \alpha, \beta, x_1, c) \gamma(g, \Theta_{Ia,g}) \mathcal{N}(0,\sigma_{Ia}),
\end{equation}
where the first  term is the SALT2 light-curve and absolute magnitude
model with a realized error snake,
and $\gamma(g, \Theta_{Ia})$ a step function to model the ``mass step''.  Recall that the  fundamental SALT2 parameters 
$\alpha$, $\beta$, and intrinsic dispersion $\sigma_{Ia}$, and the parameters that
relate SN magnitude and host,
are included in $\Theta_{Ia}$; the parameters in the SALT2 fit $t_0$, $x_1$, $c$
are included in $\theta_{Ia}$.
...
\begin{equation}
n(f, Z) = 10^{Z/2.5} \int  f(\lambda) T_0(\lambda)d\lambda,
\end{equation}
...
\item The host-galaxy information is directly encoded in the measurements
\begin{equation}
G(g)=g.
\end{equation}
\end{itemize}
...
Spectroscopic measurements are assumed
...
\label{specz:eqn}
\end{align}
People who are interested in SN spectral features
...
redshifts of all potential hosts, which is included in $G$.
For the spectroscopic sample the host is unambigous
\begin{equation}
G = g.
\end{equation}
...
signal
\begin{equation}
n \sim \mathcal{N}\left(10^{Z/2.5} \int  f(\lambda;A, B) T_0(\lambda)d\lambda,C_P\right).
\end{equation}
...
spectroscopy may depend on parameters.  These criteria have not yet been determined/selected
...
above a signal-to-noise threshold there  given $M_\nu$ available measurements:
the signal-to-noise threshold corresponding to a flux threshold
 $n_{0,\nu i}$. 

Call the likelihood of the underlying distribution without sample selection $P(A|C)$.
Let the array $b_{\nu}$ be an array that contains $0$ or $1$ based on
whether the corresponding photometric point is below or passes threshold. 
Let the larger array $b$ be a combination of the $b_\nu$ from all transients.
There are $2^{\sum_\nu M_\nu}$ possible permutations of $b$, and the underlying
likelihood can be broken up into the sum of all possible combinations
\begin{align}
p(A|C) &= \sum_k p(A, b_k| C)\\
& = \sum_k \epsilon(n, b_k)p(A| C),
\end{align}
where
\begin{equation}
\epsilon(n; b_{k})= \prod_{\nu i}\mathcal{H}\left((-1)^{b_{k\nu i}+1}(n_{k\nu i}-n_{0,\nu i})\right),
\end{equation}
...
For the analysis sample, are a smaller number of combinations of $b$ that satisfy
the selection criteria;
instead of $2^{M_\nu}$ there are $\sum_{k=N}^{M_{\nu}} \binom{M_{\nu}}{k}$ 
distinct $b_{\nu}$'s that allow acceptance of transient $\nu$.  The selection efficiency
for the complement set of possible $b$'s is zero.
The sample likelihood is then as
\begin{equation}
p(A|\mathcal{S}, C) = \frac{\sum_k \epsilon(n, b_k)p(A| C)}{\int dA\, \sum_k \epsilon(n, b_k)p(A| C)},
\end{equation}
where the sum is
over only the configurations that meet the selection criteria.

The measurement uncertainties in the light curves $n$ are independent of the other
measurements, and the measurement uncertainties are drawn from a Normal distribution:
the likelihood is assembled from truncated Gaussians with the integrals over $n$ in the numerator
proportional to error functions.
...
\begin{multline}
p(A|\mathcal{S},C) \propto
\sum_k \epsilon(n, b_k) 
\sum_T \int d\theta_{Ia}\, d\theta_{!Ia}\, dL\, p(n, {{T}}_S,{{z}}_S, \theta_{S}, G | T,  L, \Omega_M, w, r, \Theta_{Ia}, \Theta_{\mathit{!Ia}}, Z, g) \\
p(L| T, \theta_{Ia},
\theta_{!Ia},  \Omega_M, w, r, \Theta_{Ia},\Theta_{\mathit{!Ia}}, Z, g)
p(T, \theta_{Ia}
\theta_{!Ia}|  \Omega_M, w, r, \Theta_{Ia},\Theta_{\mathit{!Ia}}, Z, g).
\end{multline}


For our model and measurements
\begin{multline}
p(A|\mathcal{S},C) \propto 
\sum_k \epsilon(n, b_k) \int dL\,
p(n |  T= T_S, \theta_{T}=\theta_S, g=G, L, C)
p(L| T, \theta_{T_S}, r, \Theta_{T_S}, g)\\
\pi(r, z_S, T_S)p( \theta_{T_S}|  \Theta_{T_S},g=G) .
\end{multline}

\subsubsection{Likelihood for the {\it mis} Set}
The likelihood for the {\it mis} set follows the same pattern as that of the
{\it obs} set except for not having to account for spectroscopic data
\begin{multline}
p(n, \theta_{G} |  \mathcal{S}, C) 
 \propto \sum_k \epsilon(n; b_{k}) 
\sum_T \int d\theta_{Ia}\, d\theta_{!Ia}\, dL\, 
\sum_l
\pi_l
p(n |  g=\hat{G}_l, B, C)
p(L| T, \theta_{Ia}, \theta_{!Ia}, r, \Theta_{Ia}, \Theta_{!Ia}, g)\\
\pi(r, \hat{G}_l, T)p( \theta_{Ia},
\theta_{!Ia}|  \Theta_{Ia},\Theta_{\mathit{!Ia}}, g=\hat{G}_l).
\end{multline} 

%\begin{multline}
%p(n_{SN}, G |  \Omega_M, w, \theta_r, \alpha_{Ia}, \beta_{Ia},\sigma_{Ia}, \alpha_{\mathit{non-Ia}},\beta_{\mathit{non-Ia}}, \sigma_{\mathit{non-Ia}},  Z, g, g_N, f_g)\\
%= \sum_i \int_L dL\, p(n_{SN}, G | b_i, L, \ldots)
%P(b_{i} |L, \ldots) P(L|\ldots) ,
%\end{multline} 
%\begin{equation}
%p(n_{SN}, G | b_i, L, \ldots)
%  =  \frac{ \prod_j
% \epsilon(n_{SN,j}; b_{ij})
%  p(n_{SN}, G | L, \ldots)}

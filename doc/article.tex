...
This model captures the current treatment in current analyses but expanded to include
...
spectroscopy may be dependent on model parameters. 
...
\in b$.
The likelihood of the analysis sample is thus proportional to Eqn.~\ref{likelihood_u:eqn}
with failing configurations removed
\begin{equation}
\mathcal{L}(data|\mathcal{S}, model) \propto
\sum_{k=1}^{2^M} p(data, \hat{b}_k| \hat{b}_k
\in b, model) = \sum_{k=1}^{\prod_\nu \sum_{k=N}^{M_{\nu}} \binom{M_{\nu}}{k}} p(data, b_k| model).
\end{equation}
...
\begin{itemize}
\item Determination of flux thresholds for each photometric point given
a $S/N$ threshold
\begin{equation}
threshold(point, S/N).
\end{equation}
...
calculated by DM.
\item Identification of selection criteria for spectroscopic success, as a function
of data.
\item A rates model for transient types and subtypes
\begin{equation}
\pi(z;r).
\end{equation}
\item Decision on how to model the distribution of $x_1$ and $c$.  For a first try a
single normal distribution seems sufficient but do we want to be more ambitious?
\item A non-Ia transient model.  Much of its usage is in classification, so the PSNID
description is a place to start.
\item Calibration model and uncertainties. Assuming we work with zeropoints
$p(Z)$ is needed.
